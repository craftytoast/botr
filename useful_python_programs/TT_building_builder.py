...
def do_everything():
    ...
    for i in range(7, 12):
        try:
            if building_entries[i-7].get() != "":
                int(building_entries[i-7].get())
        except ValueError:
        ...
        if building_entries[i-7].get() == "":
            ...
            DESIRED_BUILDINGS[i] = int(building_entries[i-7].get())
...
building_labels = []
building_entries = []
for i, building in enumerate(ALL_BUILDINGS[7:]):
...
start.grid(row=8, column=0, columnspan=2)
